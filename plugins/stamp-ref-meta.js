// это стандартная стартовая часть, нужна чтобы работало даже на таких страницах где переменная $ не зарезервирована для jquery
// а используется для чего-то другого
// завершающая часть -  внизу - тоже стандартная и только для этой цели .. так вобщем принято писать плагины

(function($) {

  // Плагин jquery - это просто функция, которая регистрируется в нужном месте
  // Название плагина вставляем сюда -  stampRefMeta 

  $.fn.stampRefMeta = function(options) {

    // переменные вытягиваем из объекта options, который мы наполняем при инициализации плагина 
    ...
    // все функции заменяются на локальные переменные, которые будут функциями!! и видны только внутри объемлющей функции
    ...
    // вызываем ту самую функцию, которая делает работу
    loadMetadata();

  }; // - конец функции плагина
// завершающая часть - тоже стандартная внизу 
})(jQuery);
